...
let allowedMimeTypes = ["image/jpeg", "image/png", "application/msword", "application/vnd.openxmlformats-officedocument.wordprocessingml.document", "application/pdf"];
const postUploadSingleFile = async (req, res) => {
  ...
  if (!allowedMimeTypes.includes(mimeType)) {
    throw new Error("Only image, Word, and PDF files are allowed. Please check your file(s) (single).");
  }
...
const postUploadMultipleFiles = async (req, res) => {
  ...
  if (Array.isArray(req.files.file)) {
    for (const file of req.files.file) {
      ...
      if (!allowedMimeTypes.includes(mimeType)) {
        throw new Error("Only image, Word, and PDF files are allowed. Please check your file(s) (multiple).");
      }
    }
