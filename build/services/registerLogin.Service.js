...
const isPasswordStrong = password => {
  ...
  const numberRegex = /\d/;
  return uppercaseRegex.test(password) && lengthRegex.test(password) && specialCharRegex.test(password) && numberRegex.test(password);
};
const registerNewUser = async req => {
  try {
    ...
    if (isEmail) {
      ...
      // find roles by id
      const find_group = await GroupModel.findById(req.body.group_id).populate("_id");
      if (!find_group) {
        return {
